...
//
// ==========================
//  WEB SERIAL (ERGOMETER) SECTION
// ==========================
//
// This optional feature connects to a serial device (e.g. Kettler ergometer)
// and polls the current RPM using the ST command. The RPM is mapped to the
// locomotive speed range 0..1000 and sent to the backend. It mirrors the
// original desktop implementation but runs fully in the browser via Web Serial.

let serialPort = null;
let serialWriter = null;
let serialReader = null; // reader of decoded text
let serialReadBuffer = '';
let serialConnected = false;
let ergoPollTimer = null;

const ERGO_BAUD = 9600;
const ERGO_DATABITS = 8;
const ERGO_STOPBITS = 1;
const ERGO_PARITY = 'none';
const ERGO_FLOW = 'none';
const ERGO_POLL_MS = 1000; // same cadence as legacy app
const ERGO_CMD_MODE = 'CM\r\n';
const ERGO_STATUS_CMD = 'ST\r\n';

// Map RPM to protocol speed (0..1000). The legacy C# mapped to 0..125, so
// use a default factor of 8 (125*8 = 1000). Adjust here if needed.
let rpmToSpeedFactor = 8.0;

function setSerialBtnState(txt, enabled = true) {
  if (!serialBtn) return;
  serialBtn.textContent = txt;
  serialBtn.disabled = !enabled;
}
...
async function connectSerial() {
  ...
  try {
    // Prompt user to select a port; we keep filters open to any device.
    serialPort = await navigator.serial.requestPort({});
    await serialPort.open({
      baudRate: ERGO_BAUD,
      dataBits: ERGO_DATABITS,
      stopBits: ERGO_STOPBITS,
      parity: ERGO_PARITY,
      flowControl: ERGO_FLOW
    });

    // Writer: raw bytes encoded as UTF‑8 text
    const textEncoder = new TextEncoder();
    serialWriter = serialPort.writable.getWriter();
    // Reader: decode to UTF‑8 text and read chunks
    const decoder = new TextDecoderStream();
    const readableStreamClosed = serialPort.readable.pipeTo(decoder.writable);
    serialReader = decoder.readable.getReader();

    serialConnected = true;
    setSerialBtnState('serial ✓');

    // Enter command mode like the desktop app
    await serialWriter.write(textEncoder.encode(ERGO_CMD_MODE));

    // Start background read loop and polling timer
    startSerialReadLoop();
    startErgoPolling();
  } catch (err) {
    ...
    setSerialBtnState('serial');
    alert('Serial connection failed: ' + (err && err.message ? err.message : String(err)));
  }
}
...
function startErgoPolling() {
  stopErgoPolling();
  const textEncoder = new TextEncoder();
  ergoPollTimer = setInterval(async () => {
    if (!serialConnected || !serialWriter) return;
    try {
      await serialWriter.write(textEncoder.encode(ERGO_STATUS_CMD));
    } catch (e) {
...
function stopErgoPolling() {
  if (ergoPollTimer) {
    clearInterval(ergoPollTimer);
    ergoPollTimer = null;
  }
}
  ...
  (async () => {
    while (serialConnected && serialReader) {
      try {
        ...
        if (serialConnected) console.warn('Serial read error:', e);
        break;
      }
    }
    // If we exit the loop, ensure cleanup
    if (serialConnected) await disconnectSerial();
  })();
}

function processSerialChunk(chunkText) {
  serialReadBuffer += chunkText;
  let idx;
  while ((idx = serialReadBuffer.indexOf('\n')) >= 0) {
    let line = serialReadBuffer.slice(0, idx);
    serialReadBuffer = serialReadBuffer.slice(idx + 1);
    line = line.replace(/\r+$/, '');
    if (line) handleErgoLine(line);
  }
}

function handleErgoLine(line) {
  // Expect 8 tab‑separated fields; second is RPM per the legacy app.
  // Example: 088\t072\t324\t009\t150\t0024\t10:02\t140
  const parts = line.split('\t');
  if (parts.length < 2) return;
  const rpm = parseInt(parts[1], 10);
  if (!Number.isFinite(rpm) || rpm < 0) return;

  const speed = Math.max(0, Math.min(1000, Math.round(rpm * rpmToSpeedFactor)));
  // Update UI and backend; select a loco if none is selected yet
  if (currentLocoUid == null) {
    // try first available loco
    const keys = Object.keys(locList);
    if (keys.length) {
      const firstUid = locList[keys[0]]?.uid || Number(keys[0]);
      if (firstUid) selectLoco(firstUid);
    }
  }
  if (currentLocoUid != null) {
...
// Wire the Serial button if available; otherwise disable it with a hint
if (serialBtn) {
  if (!supportsWebSerial()) {
    serialBtn.disabled = true;
    serialBtn.title = 'Web Serial not supported in this browser (use Chrome/Edge, HTTPS or localhost)';
  } else {
